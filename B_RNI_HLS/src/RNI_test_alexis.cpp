...
void RNI_alexis(pkt_stream& in_stream, pkt_stream& out_stream)
{
...
#pragma HLS INTERFACE s_axilite port=return bundle=ctrl
	...
	//écrit le dernier buffer non-complet
	//write_probe_file();
	probe_file.close();
...
void update_membrane_probe(INDEX_TYPE neuron_index)
{
	...
	{
		if(MEMBRANE_PROBE_CURRENT_INDEX == MEMBRANE_PROBE_LENGHT-1)
			write_probe_file();
...
void update_membrane_probe(INDEX_TYPE neuron_index)
{
	std::cout << NEURONS_MEMBRANE[neuron_index] << std::endl;
	if(MEMBRANE_PROBE_CURRENT_INDEX == MEMBRANE_PROBE_LENGHT-1){
		//std::cout << neuron_index << std::endl;
		write_probe_file();
	}
	MEMBRANE_PROBE[MEMBRANE_PROBE_CURRENT_INDEX] = NEURONS_MEMBRANE[neuron_index];
	//std::cout << neuron_index << std::endl;
...
void write_probe_file(void)
{
    ...
    std::cout << MEMBRANE_PROBE_CURRENT_INDEX << std::endl;
