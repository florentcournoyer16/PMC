...
python3 $MODEL_PARSER_FILEPATH \
    --model_filepath "$MODEL_FILEPATH" \
    --tb_inputs_filepath "$TB_INPUTS_FILEPATH" \
    --weight_type_length "$WEIGHT_TYPE_LENGTH" \
    --membrane_type_length "$MEMBRANE_TYPE_LENGTH" \
    --add_membrane_probe false \
    --membrane_probe_length "$MEMBRANE_PROBE_LENGTH"
...
vivado -mode batch $VIVADO_ALL_FILEPATH \
    --generate_bitstream $GEN_BITSTREAM
